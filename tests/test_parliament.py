...
def test_build_kontakt_cql_empty_matches_all():
    # No name/party → match-all sentinel; the Paris API dropped the
    # `AktivesRatsmitglied` field, so it can no longer be filtered on.
    assert _build_kontakt_cql() == 'NameVorname any "*"'
...
@respx.mock
async def test_paris_html_error_page_is_rejected():
    """Paris renders query errors (e.g. an unknown CQL field) as an HTML page
    under HTTP 200; it must surface as the upstream message, not a ParseError."""
    respx.get(_url("kontakt")).mock(
        return_value=httpx.Response(
            200,
            headers={"content-type": "text/html; charset=utf-8"},
            content=b"<html><body>406 - Unknown index field</body></html>",
        )
    )

    result = await zurich_parliament_members(ParliamentMembersInput(party="SP"))

    assert "Fehler bei Mitgliedersuche" in result
