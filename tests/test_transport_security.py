...
def _no_inherited_allowlist(monkeypatch):
    ...
    the `env=None` paths below resolve. HOST/PORT likewise override CLI
    defaults and would make the loopback assertions lie."""
...
def test_non_loopback_bind_without_allowlist_stays_off():
    ...
    assert server._build_transport_security("::", 8000, env={}) is None
...
def test_host_defaults_to_HOST_env_when_set(monkeypatch):
    """Container platforms inject $HOST; the image binds `::` this way."""
    monkeypatch.setenv("HOST", "::")
    assert server._parse_args([]).host == "::"
    assert server._parse_args(["--http"]).host == "::"


def test_cli_host_overrides_HOST_env(monkeypatch):
    monkeypatch.setenv("HOST", "::")
    assert server._parse_args(["--http", "--host", "0.0.0.0"]).host == "0.0.0.0"


def test_port_defaults_to_PORT_env_when_set(monkeypatch):
    monkeypatch.setenv("PORT", "9000")
    assert server._parse_args([]).port == 9000


def test_cli_port_overrides_PORT_env(monkeypatch):
    monkeypatch.setenv("PORT", "9000")
    assert server._parse_args(["--http", "--port", "8001"]).port == 8001
