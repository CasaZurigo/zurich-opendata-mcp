...
def _parse_args(argv: list[str] | None = None):
    ...
    parser.add_argument(
        ...
        help=(
            "HTTP bind address (default: $HOST or 127.0.0.1; only used with "
            "--http). The loopback default is deliberate — binding 0.0.0.0 or "
            ":: exposes the server on every interface. When you do, set "
            "MCP_ALLOWED_HOSTS to the names it is reachable under, or Host "
            "validation is left to whatever fronts it."
        ),
    )
    parser.add_argument(
        "--port",
